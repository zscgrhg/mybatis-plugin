...
public class SqlTemplateParserUtil {
            ...
            if (TABLE.equalsIgnoreCase(rootElement.getName())) {
                ...
                for (Element e : list) {
                    if (COLUMN.equalsIgnoreCase(e.getName())) {
                        ...
                        String javaProperty = e.getAttributeValue(JAVA_PROPERTY);
     ...
     * @return
     */
    private static DbTableOperation parseDbTableOperation(Element e, Context context,
                                                          List<DbSelectResult> results,
                                                          Map<String, GeneratedJavaFile> maps) {
        ...
        if (null != result) {
            ...
            FullyQualifiedJavaType fullyQualifiedJavaType = getFullyQualifiedJavaType(context,
                resultType);
     ...
     * @return
     */
    private static DbSelectResult parseDbSelectResult(Element result, String resultType,
                                                      Context context,
                                                      Map<String, GeneratedJavaFile> maps) {
        ...
        if (null != objectName) {
            FullyQualifiedJavaType objectType = new FullyQualifiedJavaType(basicPackage + "."
                                                                           + objectName);
            ...
            GeneratedJavaFile generatedJavaFile = new GeneratedJavaFile(topLevelClass, context
                .getJavaModelGeneratorConfiguration().getTargetProject(), formatter);
    ...
    private static FullyQualifiedJavaType getFullyQualifiedJavaType(Context context,
                                                                    String resultType) {
        String basicPackage = context.getJavaModelGeneratorConfiguration().getTargetPackage();
        String type = resultType;
        if (null != basicPackage && !"".equals(basicPackage)
            && !resultType.startsWith(basicPackage) && resultType.indexOf('.') == -1) {
    ...
    private static boolean isBasicType(String resultType) {
        return "java.lang.Integer".equals(resultType) || "Integer".equalsIgnoreCase(resultType)
               ...
               || "java.lang.Float".equals(resultType) || "Float".equalsIgnoreCase(resultType)
               || "java.lang.Double".equals(resultType) || "Double".equalsIgnoreCase(resultType)
               || "java.lang.Boolean".equals(resultType) || "Boolean".equalsIgnoreCase(resultType);
    }

    private static void processProperty(TopLevelClass topLevelClass, String cloumn,
        ...
        String methodName = javaProperty.length() > 1 ? javaProperty.substring(0, 1).toUpperCase()
                                                        + javaProperty.substring(1) : javaProperty
            .toUpperCase();
    ...
    private static void addFieldComment(Field field, String cloumn) {
        ...
        field
            .addJavaDocLine(" * This field was generated by MyBatis Generator plugin (SqlTemplatePlugin)."); //$NON-NLS-1$
    ...
    private static void addGeneralMethodComment(Method method, String cloumn) {
        ...
        method
            .addJavaDocLine(" * This method was generated by MyBatis Generator plugin (SqlTemplatePlugin)."); //$NON-NLS-1$
    ...
    public static FullyQualifiedJavaType getFullyQualifiedJavaType(String jdbcTypeName) {
    ...
    public static String parseJdbcTypeName(String str) {
        String jdbcTypeName = str.replace("jdbcType", "").replace("=", "").trim();
        return jdbcTypeName;
    }
}
